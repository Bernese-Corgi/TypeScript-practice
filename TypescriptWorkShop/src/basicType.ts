...
/* 배열 */
let nameList: string[]; // 배열에 담길 각각의 요소들에 대한 타입을 정의
// nameList = [ 1, 4 ]; // type error
nameList = ['1', '4'];
nameList.push('6'); // 배열 메서드 사용

/* 배열: 인라인으로 타입 정의 */
let user1: { name: string; score: number }; // 변수 선언과 동시에 인라인으로 타입 정의
user1 = {
  name: 'jin',
  score: 30,
};
let user2: { name: string; score: number }; // user1과 중복되는 패턴의 배열이므로, 이 것은 interface로 정의하면 편리하다.
