...
let boolean: boolean = any; // any는 모든 타입에 할당할 수 있다.
let number1: number = unknown; // unknown 형식은 number 형식에 할당할 수 없다
let any: any = unknown; // unknown 형식은 any 형식에 할당할 수 있다.
// 타입 단언
let number2: number = unknown as number; // 타입을 단언하면 unknown 형식도 any 외의 값에 할당할 수 있다.
```
